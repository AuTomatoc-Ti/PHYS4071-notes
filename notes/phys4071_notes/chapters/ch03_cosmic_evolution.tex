...
\begin{foundation}
    ...
    \begin{align*}
        F &= \frac{L}{4\pi d_L^2}
    \end{align*}
    Let $L_0$ and $F_0$ are reference luminosity and flux, which are $L_0 = 3.0128 \times 10^{28} \mathrm{W}$ and $F_0 = \frac{L_0}{4\pi d_0^2} = 2.518 \times 10^{-8} \mathrm{W/m^2}$ with $d_0 = 10 \mathrm{pc}$. \\
    ...
    Recall that the comoving distance $D_{\text{Com}}$ is defined as:
    \[
        D_{\text{Com}} = \int_{t_e}^{t_0} \frac{dt}{a(t)} = \int_0^z \frac{dz'}{H_0 E(z')}
    \]
    where $E(z) = \sqrt{\Omega_{m}(1+z)^3 + \Omega_{r}(1+z)^4 + \Omega_{\Lambda} + \Omega_{K}(1+z)^2 + \Omega_{\Lambda}}$. \\

    At the sametime, we also know that:
    \[
        d_L = (1+z) a_0 D_{\text{Com}} = (1+z) D_{\text{Com}}
    \]
