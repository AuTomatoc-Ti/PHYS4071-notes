...
%\clearpage
\bigskip\noindent
\subsection{the Scale Factor $a(t)$}
\begin{intuition}
...
\begin{intuition}
    ...
    \[
        \Delta D_{proper} = a(t) \Delta D_{comoving}
    \]
    \\
...
\bigskip\noindent
\begin{theory}
    Let $D_{proper}$ be the proper distance to a galaxy, and $D_{comoving}$ be the co-moving distance to the galaxy, then we have:
    \[
        D_{proper}(t) = a(t) \cdot D_{comoving}
    \] 
    Note $D_{comoving}$ is independent to time.\\

    The recession velocity $v$ can also be defined as:
    \[
        v \equiv \frac{dD_{proper}}{dt} = \dot{a}(t) \cdot D_{comoving}
    \]
    And sub the Hubble's law into the above equation, we can get:
    \begin{align*}
        v &= H D_{proper} \\     
        \dot{a}(t) \cdot D_{comoving} &= H a(t) \cdot D_{comoving} \\
        \dot{a}(t) &= H a(t)
    \end{align*} \\
...
\bigskip\noindent
\begin{remark}
    Note $a(t) > 0$ for all $t$, which means the scale factor can never be zero(except at the initial singularity), while the observation shows that $H_0 > 0$, which indicates $\dot{a}(t_0) > 0$, which means the universe is expanding at the current time $t_0$.
\end{remark}
