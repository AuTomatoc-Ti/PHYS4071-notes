...
% -─────────────────────────────────────────────────────────────────────────────
\clearpage
\subsection{Luminousity Distance}
\begin{intuition}
    ...
    So we define \textbf{luminousity distance}: the distance an object \emph{appears} to have if we infer distance only from how bright it looks. \\
...
\bigskip \noindent
\begin{foundation}
  Start from the observational definition of luminousity distance:
  \[
    F \equiv \frac{L}{4\pi d_L^2},
  \]
  where $L$ is the source intrinsic luminousity (energy emitted per unit source proper time), and $F$ is the observed flux (energy received per unit detector area per unit observer time). \\
  ...
  \begin{enumerate}[label=(\roman*)]
    \item \textbf{Photon energy redshift:} from $1+z = \lambda_{obs}/\lambda_{emit} = \nu_{emit}/\nu_{obs}$,
    each photon has
    \[
    ...
    \item \textbf{Arrival-time dilation:} from previous section,
    \[
  ...
  \begin{align*}
    F
    &= \frac{L}{4\pi (a_0\chi)^2}\cdot\frac{1}{(1+z)^2} \\
    &= \frac{L}{4\pi\left[(1+z)a_0\chi\right]^2}.
  \end{align*}
...
\bigskip\noindent
\begin{theory}
    The luminousity distance $d_L$ is related to the comoving distance $\chi$ and the redshift $z$ by the formula:
    \[
        d_L = (1+z) \chi
    \]
\end{theory}
...
% \bigskip\noindent
% \begin{remark}
%     luminousity distance is not just geometric distance. It is the ``effective distance'' inferred from brightness, and expansion makes sources dimmer than pure geometry predicts. One $(1+z)$ comes from lower energy per photon, and one $(1+z)$ comes from slower photon arrival rate; together they produce the extra dimming encoded in $d_L=(1+z)\chi$. 
% \end{remark}
...
\bigskip\noindent
\begin{theory}
    Calculation on \textbf{Hubble horizon}:
    \[
        D_H(t)=\frac{c}{H(t)} \qquad (c=1 \Rightarrow D_H=1/H)
    \]
...
\bigskip\noindent
\begin{remark}
    Hubble horizon is a \textbf{kinematic scale}, not always a strict causal boundary. In many cosmological models, photons can cross the Hubble horizon when $H(t)$ changes with time.
\end{remark}
...
\begin{intuition}
    The cosmological event horizon answers a future question: \textbf{which regions can ever send a signal to us, even if we wait forever?} \\

    If a source is currently too far away, light emitted \emph{now} may never reach us because expansion can stretch space too fast at late time.
\end{intuition}
...
\bigskip\noindent
\begin{theory}
    Calculation on \textbf{event horizon}:
    \[
        \chi_{eh}(t)=\int_t^{\infty}\frac{dt'}{a(t')},
        \qquad
        D_{eh}(t)=a(t)\int_t^{\infty}\frac{dt'}{a(t')}.
    \]
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.75\textwidth]{images/penrose_event_horizon.png}
    \caption{Penrose diagram illustrating the concept of cosmological event horizon.}
\end{figure}
...
\begin{intuition}
    The particle horizon answers a past question: \textbf{how far can we see by now?} \\
...
\begin{foundation}
    ...
    \[
        \chi_{ph}(t)=\int_{0}^{t}\frac{dt'}{a(t')}.
    \]
    Thus the proper particle-horizon distance is
    \[
        D_{ph}(t)=a(t)\chi_{ph}(t)=a(t)\int_{0}^{t}\frac{dt'}{a(t')}.
    \]
\end{foundation}
...
\bigskip\noindent
\begin{theory}
    Calculation on \textbf{particle horizon}:
    \[
        \chi_{ph}(t)=\int_{0}^{t}\frac{dt'}{a(t')},
        \qquad
        D_{ph}(t)=a(t)\int_{0}^{t}\frac{dt'}{a(t')}.
    \]
    At present time $t_0$:
    \[
        \chi_{ph,0}=\int_{0}^{t_0}\frac{dt'}{a(t')},
        \qquad
        D_{ph,0}=a_0\chi_{ph,0}=\chi_{ph,0}\quad(a_0=1).
    \]
\end{theory}

\bigskip\noindent
\begin{remark}
    Particle horizon is about \textbf{past light cone} (what has been observable), while event horizon is about \textbf{future light cone} (what can ever become observable).
\end{remark}

\bigskip\noindent
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.75\textwidth]{images/penrose_particle_horizon.png}
    \caption{Penrose diagram illustrating the concept of cosmological particle horizon.}
\end{figure}

\clearpage
%\bigskip\noindent
\begin{example}{Numerical Comparison of Three Horizons in Our Universe}{three-horizons-lcdm}
    Use a flat $\Lambda$CDM background with present parameters close to Planck values:
    \[
        H_0\approx 67.66\,\mathrm{km\,s^{-1}\,Mpc^{-1}},\qquad
        \Omega_m^0\approx 0.315,\qquad
        \Omega_\Lambda^0\approx 0.685.
    \]

    (a). Estimate present-day values of the three horizons. \\
    (b). Compare their order and explain the physical meaning. \\
    (c). In the limit $t\to\infty$, discuss what happens to the three horizons and whether the order swaps. \\

    \bigskip\textbf{Solution.}\quad \\
    (a). \textbf{Present-day values} ($t=t_0$): \\
    \textit{Hubble horizon}
    \[
        D_H(t_0)=\frac{c}{H_0}
        \approx \frac{299792\,\mathrm{km/s}}{67.66\,\mathrm{km\,s^{-1}\,Mpc^{-1}}}
        \approx 4.43\,\mathrm{Gpc}
        \approx 14.4\,\mathrm{Gly}.
    \]

    \textit{Particle horizon}
    \[
        D_{ph}(t_0)=a_0\int_0^{t_0}\frac{dt'}{a(t')}
        \quad (a_0=1).
    \]
    Numerically (for the above $\Lambda$CDM parameters):
    \[
        D_{ph}(t_0)\approx 14.2\,\mathrm{Gpc}\approx 46.3\,\mathrm{Gly}.
    \]

    \textit{Event horizon}
    \[
        D_{eh}(t_0)=a_0\int_{t_0}^{\infty}\frac{dt'}{a(t')}
        \quad (a_0=1).
    \]
    Numerically:
    \[
        D_{eh}(t_0)\approx 5.0\,\mathrm{Gpc}\approx 16.3\,\mathrm{Gly}.
    \]
    ...
    (b). \textbf{Physical intuition for this order}: \\
    ...
    (c). \textbf{Future limit} $t\to\infty$ in our $\Lambda$-dominated universe: \\
    ...
    So
    \[
        D_H(t)\to \frac{c}{H_\Lambda},
        \qquad
        D_{eh}(t)\to \frac{c}{H_\Lambda},
    \]
    i.e. Hubble horizon and event horizon approach the same constant (about $17\,\mathrm{Gly}$ for the above parameters). \\
    Meanwhile
    \[
        D_{ph}(t)=a(t)\int_0^t\frac{dt'}{a(t')}\to\infty,
    \]
    ...
    \]
    So in our universe there is \textbf{no full order swap}; instead, the present inequality $D_{eh}>D_H$ gradually shrinks to equality.
\end{example}
